...
void scale_image(Image& im, int c, float v)
  ...
  // cambiare contrasto
  //scalare vuol dire fare il prodotto
  ...
  for(int i=0;i<im.w;i++){
      for(int j=0;j<im.h;j++){
          for(int z=0;z<im.c;z++) {
              float value = im.data[pixel_address(im,i, j, z)];
              if(value<0)
                  im.data[pixel_address(im,i, j, z)]=0;
              else if(value>1)
                  im.data[pixel_address(im,i, j, z)]=1;
          }
      }
  }
...
void rgb_to_hsv(Image& im)
  ...
  float& G= RGB[1];
  float& B= RGB[2];
  int index_value;  // cerco il valore massimo di un pixel tra i 3 canali di cui è composto
  for(int i=0;i<im.w;i++){
      for(int j=0;j<im.h;j++) {
          RGB[0] = im(i, j, 0);
          RGB[1] = im(i, j, 1);
          RGB[2] = im(i, j, 2);
          //---calcolo la componente VALUE
          index_value = max_element(RGB, RGB + 3) - RGB;
          V = RGB[index_value];
          //--calcolo la componente Saturazione
          m = *min_element(RGB, RGB + 3);  //puntato dal puntatore all'elemento più piccolo
          C = V - m;
          if (V == 0) {
              S = 0; //se V nullo la saturazione è nulla
          }
          else{
              S=C/V ;
          }
          //calcolo la componente HUE
          ...
          //adesso scriviamo i valori nell'immagine
          im(i,j,0)=H;
          im(i,j,1)= S;
          im(i,j,2)=V;
...
void hsv_to_rgb(Image& im)
  ...
  for(int i=0;i<im.w;i++){
      for(int j=0;j<im.h;j++){
          ...
          //aggiorno i valori dell'immagine
          im(i,j,0)=R;
          im(i,j,1)=G;
          im(i,j,2)=B;
      }
